...
class RhinoThrustDiagram(RhinoDiagram):
    ...
    def draw(self, settings):
        self.artist.layer = settings.get('thrust.layer')
        self.artist.clear_layer()

        print(settings)
        print(self.artist.layer)

        if settings.get('thrust.show.vertices', True):
            keys = list(self.diagram.vertices_where({'is_external': False}))
            color = {}
            color.update({key: settings.get('thrust.color.vertices') for key in self.diagram.vertices()})
            color.update({key: settings.get('thrust.color.vertices:is_fixed') for key in self.diagram.vertices_where({'is_fixed': True})})
            color.update({key: settings.get('thrust.color.vertices:is_anchor') for key in self.diagram.vertices_where({'is_anchor': True})})
            self.guid_vertices = self.artist.draw_vertices(color=color, keys=keys)

        if settings.get('thrust.show.edges', True):
            keys = list(self.diagram.edges_where({'is_edge': True, 'is_external': False}))
            color = {}
            color.update({key: settings.get('thrust.color.edges') for key in keys})
            self.guid_edges = self.artist.draw_edges(keys=keys, color=color)

        if settings.get('thrust.show.faces', True):
            keys = list(self.diagram.faces_where({'is_loaded': True}))
            color = {}
            color.update({key: settings.get('thrust.color.faces') for key in keys})
            self.guid_faces = self.artist.draw_faces(keys=keys, color=color)

        if settings.get('thrust.show.external', True):
            self.artist.draw_external(scale=settings.get('thrust.scale.external', 1.0))
